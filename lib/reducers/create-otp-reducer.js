...
import { addToSearches, getISOLikeTimestamp } from '../util/state'
      ...
      case 'VEHICLE_RENTAL_RESPONSE':
        return update(state, {
          overlay: {
            vehicleRental: {
              pending: { $set: false },
              stations: { $set: action.payload.data.rentalVehicles }
            }
          }
        })
      ...
      case 'ADD_LOCATION_SEARCH':
        return update(state, {
          location: {
            sessionSearches: {
              $set: addToSearches(
                state.location.sessionSearches,
                action.payload.location
              )
            }
          }
        })
      case 'SET_LIVE_NAVIGATION_ENABLED':
        return update(state, {
          location: {
            liveNavigationEnabled: { $set: action.payload }
          }
        })
      case 'SET_WATCH_POSITION_ID':
        return update(state, {
          location: {
            watchPositionId: { $set: action.payload }
          }
        })
