...
class ResponsiveWebapp extends Component {
  ...
  componentDidMount() {
    ...
    if (isMobile()) {
      ...
      // Start continuous position tracking using watchPosition for live navigation
      // if it's enabled in the config (defaults to true on mobile)
      const liveNavigationEnabled =
        this.props.config?.liveNavigation?.enabled !== false
      if (liveNavigationEnabled) {
        startWatchingPosition(intl)
      }
    }
  ...
  componentWillUnmount() {
    ...
    const { stopWatchingPosition } = this.props
...
const mapDispatchToProps = {
  ...
  startWatchingPosition: locationActions.startWatchingPosition,
  stopWatchingPosition: locationActions.stopWatchingPosition
}
